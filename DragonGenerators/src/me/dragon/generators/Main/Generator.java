package me.dragon.generators.Main;
...
public class Generator implements Serializable {
    ...
    public void upgrade(Main plugin, Player p) {
        if(this.level <= 4) {
            ...
            if ((generatortoremove = getGeneratorAt(plugin, location)) != null) {
                ...
                plugin.saveData();
                p.sendMessage(Utils.color(Main.prefix + plugin.getConfig().getString("upgradesuccessmessage").replace("%LEVEL%", String.valueOf(this.getLevel()))));
            }
        } else {
            p.sendMessage(Utils.color(Main.prefix + plugin.getConfig().getString("maxlevelmessage")));
        }
    }
    ...
    public void openupgradegui(Main plugin, Player p) {
        Inventory inventory = Bukkit.createInventory(null, 27, Utils.color("&bUpgrade Menu!"));
        ...
        genmeta.setDisplayName(Utils.color(plugin.getConfig().getString("genname")));
        genmeta.setLore(Arrays.asList(Utils.color("&bLevel: " + this.level),
                Utils.color("&bTime: " + this.getTime(plugin) + " second(s)"),
                Utils.color("&bAmount dropped: " + plugin.getConfig().getInt("generatoramounts.level" + this.getLevel()))));
        gen.setItemMeta(genmeta);
        inventory.setItem(13, gen);

        ItemStack upgrade = new ItemStack(Material.NETHER_STAR);
        ItemMeta upgrademeta = upgrade.getItemMeta();
        upgrademeta.setDisplayName(Utils.color(plugin.getConfig().getString("upgradename")));
        upgrademeta.setLore(Arrays.asList(Utils.color("&bNext Level: " + (this.level + 1)),
                Utils.color("&bTime: " + plugin.getConfig().getInt("generatorintervals.level" + (level + 1)) + " second(s)"),
                Utils.color("&bAmount dropped: " + plugin.getConfig().getInt("generatoramounts.level" + (this.getLevel() + 1))),
                Utils.color("&bPrice: $" + plugin.getConfig().getInt("level"+((this.level < 5 ? this.level + 1 : 0))))));
        ...
        destroymeta.setDisplayName(Utils.color(plugin.getConfig().getString("destroyname")));
        destroymeta.setLore(Arrays.asList(Utils.color("&bThis will put the item back in your inventory"),
                Utils.color("&bBut next time you place it, it will be level 1.")));
    ...
    public static void giveGenerator(Main plugin, Player p, short level) {
        ...
        itemMeta.setDisplayName(Utils.color(plugin.getConfig().getString("genname"))); //TODO: add to config and line 81
            ...
            if(plugin.getConfig().getBoolean("logbreakingandplacing")) {
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    if (player.isOp() || player.hasPermission("gens.notify")) {
                        player.sendMessage(Utils.color("&c&lGenerator &cbroken at " + location.toString() + " by " + p.getName() + " level: " + generatortoremove.level));
                    }
                }
            }
            if(p == null) {
